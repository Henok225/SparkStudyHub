...
import { BookText, Search, Star, ChevronDown, FileQuestionMark } from 'lucide-react';
import '../LessonLister/CurLessonsLister.css'
import { useNavigate, useParams } from 'react-router-dom';
import axios from 'axios';
import { StoreContext } from '../../Context/StoreContext';

const CurQuizzesLister = () => {
  ...
  const [quizzesList, setQuizzesList] = useState({ loaded: false, content: [] });
  const [filteredquizzesList, setFilteredquizzesList] = useState({ content: [] })
...
useEffect(() => {
      ...
      try {
        ...
        const filteredData = response.data.data.filter((item)=>item.grade === gradeId && (item.subject.toLowerCase() === selectedSubject.toLowerCase() || selectedSubject.toLowerCase() === "all subjects")); // filtering by selected grade
         ...
         filteredData.forEach((element) => {
          const searchKey1 = element.title.toLowerCase();
        ...
        setQuizzesList({ loaded: true, content: filteredData })
        setFilteredquizzesList({ content: filteredData })
    ...
    // }
  }, [refetchquizzes, selectedSubject])
      ...
      <div className="spark-header">
        ...
        <div className="spark-header-right">
          <div className="spark-breadcrumb">
           <span onClick={()=>navigate("/")}> Home</span> &gt; <span onClick={()=>navigate('/ethiopian-curriculum')}>Curriculum</span> &gt; <span>Ethiopian</span> &gt; <span>Grade 9</span> &gt; <span>quizzes</span>
          </div>
        </div>
      </div>

      <div className="spark-content">
        <h1 className="spark-page-title">Grade {gradeId} - quizzes</h1>

        <div className="spark-filters-bar">
          <div className="spark-search-container">
            <span className="spark-search-icon"><Search size={20} /></span>
            <input onChange={handleSearch} type="text" placeholder="Search for quizzes..." className="spark-search-input" />
        ...
        <div className="spark-cards-grid">
          {loading ? (
            ...
            {filteredquizzesList && filteredquizzesList.content.length !== 0 ?
                filteredquizzesList.content.map((card, index) => (
                    <div
                      key={card._id}
                    ...
                    >
                      ...
                      <div className="spark-card-header">
                        <div className="spark-card-icon-wrapper">
                          <span className="spark-card-icon"><FileQuestionMark size={24} /></span>
                        </div>
                      ...
                      <div className="spark-card-footer">
                        <button onClick={() => { token ? navigate(`/quizzes/${card.subject}/${card._id}`) : setShowLogin(true) }} className="spark-view-lesson-button">View</button>
...
export default CurQuizzesLister;
